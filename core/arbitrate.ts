...
export async function arbitrate({
  ...
  const [exchange1, exchange2] = instantiateExchanges(
    exchanges,
    orderBooks,
    symbol,
  )
...
function instantiateExchanges(
  ...
  const [exchange1, exchange2] = R.zip(exchanges, orderBooks).map(
    ([exchange, orderBook]) => {
...
function fetchOrderBooks(
  ...
  return Promise.all(
    exchanges.map(exchange =>
      exchangeClient.fetchOrderBook(exchange.name, symbol),
    ),
  )
}

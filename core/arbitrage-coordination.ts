...
export default class ArbitrageCoordination {
  ...
  public async arbitrate() {
    ...
    if (!market1.isLiquid) {
      throw new Error(
        `One of the markets is illiquid ${JSON.stringify(market1, null, 4)}`,
      )
    }

    if (!market2.isLiquid) {
      throw new Error(
        `One of the markets is illiquid ${JSON.stringify(market2, null, 4)}`,
      )
    }

    const { assessment1, assessment2 } = this.assess({
      symbol: this.symbol,
      exchange1,
      exchange2,
    })
    await Promise.all([
      this.assessmentRepository.save(assessment1),
      this.assessmentRepository.save(assessment2),
    ])
  }

  private instantiateExchanges(orderBooks: OrderBook[]): [Exchange, Exchange] {
    const [exchange1, exchange2] = R.zip(this.exchanges, orderBooks).map(
      ([exchange, orderBook]) => {
